        ...
        if self.transaction.autorefill.pre_refill_sms:
            if not self.transaction.customer_confirmation:
                if not self.transaction.check_sms_confirmation():
                    ...
                    self.transaction.adv_status = 'Canceled. Transaction wasn`t confirmed via SMS'
                    self.transaction.save()
                    return self.transaction
                else:
                    self.transaction.adv_status = 'Transaction confirmed via SMS'
            ...
            self.transaction.adv_status = 'Transaction doesn`t need confirmed via SMS'
